...
// Helper function for recursive right view traversal.
void rightViewHelper(Node* root, vector<int> &ans, int level) {
    if(!root) {
        return;
    }
    // Check if the current level is equal to the size of ans vector.
    // If they are equal, it means this is the rightmost node at this level.
    if(ans.size() == level) {
        // Add the value of the rightmost node to the ans vector.
...
vector<int> rightViewRecursively(Node *root) {
    vector<int> ans;
    ...
    ans.push_back(root->value);
    // Start the recursive traversal from the right child.
    rightViewHelper(root->right, ans, 1);
    // Then traverse the left child.
    rightViewHelper(root->left, ans, 1);
    // Return the vector containing the right view nodes.
    return ans;
}

//Approach 2: Function to print the Right View of a binary tree using an iterative approach
vector<int> rightViewIteratively(Node *root) {
    vector<int> ans;
    // If the tree is empty, return an empty result.
    if(!root) {
        return ans;
...
int main() {
    ...
    cout<<"The Right View Of Binary Tree using Recursive Approach: "<<endl;
    vector<int> ans = rightViewRecursively(root);
    for(int a : ans) {
        cout<<a<<" ";
    } cout<<endl;

    cout<<"The Right View Of Binary Tree using Iterative Approach: "<<endl;
    ans = rightViewIteratively(root);
    for(int a : ans) {
        cout<<a<<" ";
    } cout<<endl;
