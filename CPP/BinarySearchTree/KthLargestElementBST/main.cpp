...
int kthLargestElement(Node* root, int k) {
    ...
    // Return the kth largest element (0-based index)
    return inorderAns[inorderAns.size() - k];
}

// Helper function for finding the kth Largest Element
int kthLargestElementHelper(Node* root, int &index, int k) {
    if(!root) {
        // Return -1 if the tree is empty
        return -1;
    }
    int rightAns = kthLargestElementHelper(root->right, index, k);
    if(rightAns != -1) {
        // If the right subtree has the kth largest element, return it
        return rightAns;
    }
    if(++index == k) {
    ...
    kthLargestElementHelper(root->left, index, k);
}

//Approach 2: Find the kth Largest Element in a Binary Search Tree (Optimized)
int kthLargestElementOptimized(Node* root, int k) {
    if(!root) {
        // Return -1 if the tree is empty
        return -1;
    }
    ...
    return kthLargestElementHelper(root, index, k);
}
...
int main() {
    ...
    int k = 3;
    cout<<"The "<<k<<" Largest Element of BST: "<<kthLargestElement(root, k)<<endl;

    cout<<"The "<<k<<" Largest Element of BST: "<<kthLargestElementOptimized(root, k)<<endl;
