...
//Approach 1: Function to calculate the average values of levels in a binary tree
vector<double> averageOfLevels(Node* root) {
    ...
    q.push(root);
    double average;
    vector<double> ans; // Vector to store average values
    while(!q.empty()) {
        // Get the number of nodes at the current level
        int size = q.size();
...
int main() {
    ...
    cout<<"The Average of Levels: "<<endl;
