...
// Helper function to convert a BST to a Max Heap
Node* convertBSTToMaxHeapHelper(Node* root, vector<int> &ans, int &index) {
    if(!root) {
        // Base case: Return null for an empty node
        return nullptr;
    }
    // Traverse the tree in reverse in-order and assign values from the vector
    root->value = ans[index--];
    // Recursively convert the right and left subtrees
    root->right = convertBSTToMaxHeapHelper(root->right, ans, index);
    root->left = convertBSTToMaxHeapHelper(root->left, ans, index);
    // Return the modified node
    return root;
}

//Approach 1: Main function to convert a BST to a Max Heap
void convertBSTToMaxHeap(Node* root) {
    ...
    // Initialize the index to the end of the vector
    int index = inorderAns.size() - 1;
    // Call the helper function to perform the conversion
    convertBSTToMaxHeapHelper(root, inorderAns, index);
}   
...
int main() {
    ...
    cout<<endl<<"Converting The BST To Max Heap: "<<endl;
    convertBSTToMaxHeap(root);

    cout<<"The Level Order Traversal of Max Heap: "<<endl;
    levelOrderTraversal(root);

    cout<<endl<<"The Inorder Traversal of Max Heap: "<<endl;
    inorderTraversal(root);
    cout<<endl;

    cout<<endl<<"The Preorder Traversal of Max Heap: "<<endl;
    preorderTraversal(root);
    cout<<endl;

    cout<<endl<<"The Postorder Traversal of Max Heap: "<<endl;
    postorderTraversal(root);
    cout<<endl;
