...
// Helper function for counting non-leaf nodes (nodes with at least one child) recursively
void inorderCount(Node* root, int &count) {
    ...
    if(root->left || root->right) {
        // Increment the count of non-leaf nodes
        count++;
    }
...
int countNonLeavesRecursively(Node* root) {
    int count = 0;
...
int countNonLeavesIteratively(Node* root) {
    if(!root) {
        return 0;
    }
    stack<Node*> st;
...
int main() {
    ...
    cout<<"The Non Leaves Count Recursively: "<<countNonLeavesRecursively(root)<<endl;
    cout<<"The Non Leaves Count Iteratively: "<<countNonLeavesIteratively(root)<<endl;
