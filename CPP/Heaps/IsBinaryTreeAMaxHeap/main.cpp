...
// Function to check if the binary tree is a max heap recursively
bool isMaxHeapRecursively(Node* root) {
    if(!root || !root->left && !root->right) {
        // Leaf nodes and empty trees are max heaps by definition
        return true;
    }
    // If the left child exists, but the right child is missing
    if(root->left && !root->right) {
        // If the parent's value is greater than the left child's value, it's still a max heap
    ...
    // If both left and right children exist and parent value is greater than both childs its Max Heap
    ...
    // Recursively check the left and right subtrees for max heap property
    bool isLeftHeap = isMaxHeapRecursively(root->left);
    bool isRightHeap = isMaxHeapRecursively(root->right);
    // Ensure both left and right subtrees satisfy the max heap property
...
bool isHeapRecursive(Node* root) {
    ...
    // Check if the tree is complete and follows max heap properties recursively
    return (isCompleteTree(root) && isMaxHeapRecursively(root));
}

// Function to check if the binary tree is a max heap iteratively
bool isMaxHeapIteratively(Node* root) {
    if(!root || !root->left && !root->right) {
        // Leaf nodes and empty trees are max heaps by definition
        return true;
    }
...
bool isHeapItartive(Node* root) {
    ...
    // Check if the tree is complete and follows max heap properties iteratively
    return (isCompleteTree(root) && isMaxHeapIteratively(root));
}

// Function to check if the binary tree is a complete max heap
bool isCompleteMaxHeap(Node* root) {
    queue<Node*> q;
    q.push(root);
    bool notCBT = false;
    while(!q.empty()) {
        ...
        q.pop();
        if(currNode) {
            // If a null node is encountered before a non-null node, it's not a complete max heap
            if(notCBT) {
                return false;
            }
            // If the left child exists
            if(currNode->left) {
                // parent value is less than left child it not a max-heap
                if(currNode->value < currNode->left->value) {
                    return false;
                }
            }
            // If the right child exists
            if(currNode->right) {
                // parent value is less than right child it not a max-heap
                if(currNode->value < currNode->right->value) {
                    return false;
                }
            }
...
bool isHeapOptimized(struct Node* root) {
    ...
    // Check if the tree is a complete max heap using an optimized approach
    return isCompleteMaxHeap(root);
}
